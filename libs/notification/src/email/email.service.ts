    ...
    if (params && params.length) {
      for (let i = 0; i < params.length; i++) {
        ...
        if (param.channel === 'aws' && !aws) {
          throw new InternalServerErrorException(new Error('aws config is required to send email through aws channel'));
        }
        if (param.channel === 'api' && !api) {
          throw new InternalServerErrorException(new Error('api config is required to send email through api channel'));
        }
        const from = param.from ?? this.emailNotificationOptions.from;
        if (!from) {
          throw new InternalServerErrorException(new Error('from is required to send email notification'));
        }
        param.from = from;
        if (param.template) {
          const template = param.template;
          const htmlPath = path.join(__dirname, '..', template.hbsHtmlTemplatePath);
        ...
        if (param.channel === 'aws' && aws) {
          ...
          emailResponse.push(await this.awsService.sendEmailThroughSES(param, SES));
        }
        if (param.channel === 'api' && api) {
      ...
      throw new InternalServerErrorException(new Error('email params is required to send email notification'));
    }
