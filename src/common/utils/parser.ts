...
import { CanExportResponseData, CanExportResponseOption } from '@can/common/types/export-response.type';
...
export function decodeUriValues(value: string, isLimitFilterRequired: boolean = false) {
  try {
 ...
 * @param query : any
 */
export function parseQueryToSequalizeQuery(query: any, isLimitFilterRequired: boolean): any {
  if (!query.hasOwnProperty('limit') && !excelExport(query) && !isLimitFilterRequired) {
    query['limit'] = process.env.REQUEST_LIMIT ? process.env.REQUEST_LIMIT : 200;
  }
...
function excelExport(query: any) {
  const index = query && query['where'] && query['where']['and'] ? query['where']['and'].findIndex((q: any) =>
    q.hasOwnProperty('exportExcel'),
  ) : -1;
  if (query.hasOwnProperty('exportExcel') || query['where']['exportExcel'] || (index >= 0 && query['where']['and'][index]['exportExcel'])) return true;
}
