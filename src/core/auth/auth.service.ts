...
export class AuthService {
  ...
  async generateToken(
    ...
    this.redisService.setWithExpiryTime(
      this.redisKeysService.userTokenKey(tokenData?.userId),
      data,
      expiryTime,
    );
    return result;
    ...
    if (social.channel == SocialChannel.GOOGLE) {
      try {
        ...
        const { access_token: accessToken } =
          await this.googleService.getLoginResponse(social.accessToken);
        const userInfo = await this.googleService.getUserProfile(accessToken);
        const user = await this.userService.findOne({
